...
class WikiDiff3 {
  ...
  public function diff(array $from, array $to) {
    ...
    if ($this->m == 0 || $this->n == 0) {
      ...
      for ($forwardBound = 0; $forwardBound < $max && $this->from[$forwardBound] === $this->to[$forwardBound]; ++$forwardBound) {
      ...
      $this->length = $forwardBound + $this->m - $backBoundL1 - 1 + $this->lcs_rec(
        $forwardBound,
        $backBoundL1,
        $forwardBound,
        $backBoundL2,
  ...
  private function lcs_rec($bottoml1, $topl1, $bottoml2, $topl2, &$V, &$snake) {
    ...
    if ($d > 1) {
      return $len + $this->lcs_rec($bottoml1, $startx - 1, $bottoml2, $starty - 1, $V, $snake)
    ...
    if (($delta & 1) == 1) {
      for ($d = 0; $d <= $limit; ++$d) {
        ...
        for ($k = $start_diag; $k <= $end_diag; $k += 2) {
          if ($k == -$d || ($k < $d && $V0[$limit_min_1 + $k] < $V0[$limit_plus_1 + $k])) {
            $x = $V0[$limit_plus_1 + $k];
          }
          else {
        ...
        for ($k = $start_diag; $k <= $end_diag; $k += 2) {
          if ($k == $d || ($k != -$d && $V1[$limit_min_1 + $k] < $V1[$limit_plus_1 + $k])) {
            $x = $V1[$limit_min_1 + $k];
          }
          else {
          ...
          $snake2 = 0;
          while ($x > 0 && $y > 0 && $from[$x + $bottoml1_min_1] === $to[$y + $bottoml2_min_1]) {
            --$x;
            --$y;
            ++$snake2;
          }
      ...
      for ($d = 0; $d <= $limit; ++$d) {
        ...
        for ($k = $start_diag; $k <= $end_diag; $k += 2) {
          ...
          if ($k >= -$delta - $d && $k <= $d - $delta && $x <= $V0[$limit + $k + $delta]) {
            $snake0 = $bottoml1 + $x;
            $snake1 = $bottoml2 + $y;
  ...
  private static function findMostProgress($M, $N, $limit, $V) {
    ...
    // the 1st entry is current, it is initialized
    $num_progress = 0;
    // with 0s
