...
class DifferenceEngine extends ContextSource {
  ...
  private function showMissingRevision() {
    ...
    $missing = array();
    if ($this->mOldRev === NULL || ($this->mOldRev && $this->mOldContent === NULL)) {
      $missing[] = $this->deletedIdMarker($this->mOldid);
    }
    if ($this->mNewRev === NULL || ($this->mNewRev && $this->mNewContent === NULL)) {
  ...
  public function generateContentDiffBody(Content$old, Content$new) {
    ...
    if (!($new instanceof TextContent)) {
      throw new MWException("Diff not implemented for " . get_class($new) . "; " . "override generateContentDiffBody to fix this."
      );
    }
  ...
  public function addHeader($diff, $otitle, $ntitle, $multi = '', $notice = '') {
    ...
    if (!$diff && !$otitle) {
      ...
      // Safari/Chrome show broken output if cols not used
      if ($diff) {
        $header .= "
